...
function Todos({ isAuthenticated, setIsAuthenticated }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (filter === 'Completed') {
          url = `http://localhost:8080/api/todo/${pageNumber - 1}/${pageSize}?isCompleted=true`;
        } else if (filter === 'Not Completed') {
          url = `http://localhost:8080/api/todo/${pageNumber - 1}/${pageSize}?isCompleted=false`;
        }
  ...
  const markCompleted = async (id) => {
    try {
      await axios.put(`http://localhost:8080/api/todo/${id}/markcomplete`, {}, {
        headers: {
        ...
        <tbody>
          {
            todos.map((todo) => {
              return <tr className={todo.isCompleted ? 'completed' : ''} key={todo.id}>
                ...
                <td><button className="btn btn-danger" onClick={() => deleteTodo(todo.id)}>Delete
</button></td>
            </tr>
          })
        }
